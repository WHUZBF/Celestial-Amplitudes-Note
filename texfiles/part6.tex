...
\begin{theorem}
	...
	\end{equation}
	这里$\mathbb{Z_0^+}$的意思是正整数以及0，后面的乘积$\prod$在$s=0(\text{or }r=0)$时定义为1，上面的系数形式还有一个非常重要的信息，阿讷就是只有当$h_k<h_i+h_j$时才有贡献。
\end{theorem}
...
\begin{theorem}[Generalized Wick]
	\begin{equation}
		{A(z)(BC)}(w)=\frac1{2\pi i}\oint_{w}\frac{dx}{x-w}\left\{\wick{\c A(z)\c B}(x)C(w)+B(x)\wick{\c A(z)\c C}(w)\right\}+\text{regular}
	\end{equation}
\end{theorem}
