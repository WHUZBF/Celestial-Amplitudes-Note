...
\end{definition}
\begin{table}[H]\label{descendant}
\centering
\begin{tabular}{lrc}
...
\end{theorem}
\begin{proof}
	利用态算符对应，取$w\to 0$的极限就能看出正确性了。代入OPE实际计算几个粒例子也能得到表\ref{descendant}的结果。
\end{proof}
...
\begin{theorem}
	...
	\begin{equation}\label{33.31}
		\boxed{
			\begin{gathered}
				\left\langle\widehat{L}_{-n}\left.\phi(w)\left.\phi_1(w_1)\ldots\phi_N(w_N)\right.\right\rangle=\mathcal{L}_{-n}\left\langle\left.\phi(w)\left.\phi_1(w_1)\ldots\phi_N(w_N)\right.\right\rangle\right.\right. \\
