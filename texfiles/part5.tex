...
\begin{figure}[htbp]
	...
	\label{fig:11}
\end{figure}
考虑匀速运动的有质量粒子，$\vec{r}=\frac{\vec{p}}{E}t+{\vec{r}_{0}}$：
\begin{equation}
	\begin{aligned}
...
类似于\ref{eq:24.2}，可以得到：\sn{注意这里不使用上标in，out来标记出入射，螺旋度、电荷、能量的大小用$h,Q,p^0$表示，那么出射取正，入射粒子的物理量为$-h,-Q,-p^0$，也就是在前面加了个$\eta_n$被我们省略了}
\begin{equation}
	\begin{aligned}
...
\end{equation}
满足运动方程：\sn{取归一化\[f^{acd}f^{bcd}=\delta^{ab}=\operatorname{tr}\bigl[T^aT^b\bigr]\]}
\begin{equation}
...
\begin{equation}
	...
	\end{gathered}
\end{equation}
上面是纯粹的渐近分析，我们平时说的规范变换在无穷远处归0，这自然是保护前面场强的渐近展开的，但实际上还有下面的无穷远处不归0的large gauge变换也不会改变场强的渐近展开形式：
\begin{equation}
...
胶子同样会有Ward恒等式$q_\mu\mathcal{M}^\mu=0$，所以考虑将\ref{eq:28.24}中$\epsilon\mapsto q$，这导致不被禁闭的散射过程必有：
\begin{equation}
