...
\end{equation}
这里$b$可以从$-s$取到$s$。Conformal basis可以一般的写为下面的形式：
...
\end{equation}
但是这一条件存在对维数是有约束的。Majorana条件还可以叙述为Dirac共轭和Majorana共轭相等。\begin{margintable}\footnotesize 
	\begin{tabularx}{\marginparwidth}{|X}
		$\bar{\lambda}\equiv\lambda^T\mathcal{C}$\\
		$\bar{\lambda}^C\equiv\lambda^\dagger A\alpha^{-1}$
	\end{tabularx}
\end{margintable}
